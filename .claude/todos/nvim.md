...
- **Phase 2 — incremental modernizations** (each walked through with the `didactic-upstream-diff-iteration` skill, one category at a time, adopt/decline/test/commit):
  ...
  - Org-mode editor support (nvim-orgmode/orgmode — outline nav primary, full org capabilities desired)
...
| Treesitter API | **✅ Done 2026-05-25** (commit `5c9fd08a`). Migrated to `main` branch + new API. Parsers auto-install at startup via `require('nvim-treesitter').install({...})`; FileType autocmd calls `vim.treesitter.start()` to enable highlighting. nvim-compe `treesitter` source temporarily disabled — proper fix lands with the cmp migration (step 2 below). Required `pkgs.tree-sitter` in common.nix because the main branch compiles parsers from source. **Follow-up 2026-05-29:** the `Plug` line originally didn't pin a branch, so the checkout silently rode the remote's default `HEAD` — which is `main` only by chance. A re-clone landed it on the legacy `master` branch instead, which lacks `install()`, throwing `attempt to call field 'install' (a nil value)` at startup. Fixed by pinning `{'branch': 'main', ...}` explicitly (deterministic across machines); switching an already-checked-out plugin's branch needs the `rm -rf plugged/<plugin>` + `PlugInstall` dance per CLAUDE.md. | — | 🟢 | 1 |
...
| Status line | **✅ Done 2026-05-25**. Replaced vim-airline with **lualine.nvim**. Landed in two commits to keep the experimental bit revertable: `36b6dfe6` swapped the plugin at the same minimal section layout (mode + filename only, matching the four blanked airline sections), then a follow-up tuned the config — Powerline triangle separators (`section_separators` + `component_separators`) emptied since they didn't fit the colorscheme, and high-signal components added to `lualine_x`: `diff` (added/modified/removed from gitsigns) and `diagnostics` (LSP error/warning counts). Both render nothing on clean state, so quiet buffers stay quiet. A third, `searchcount` (`[N/M]` while a search is hot), was tried in the same tuning commit but **removed 2026-05-29 — disliked in practice**, leaving the two above. Filetype icons left off — would have required pulling in `nvim-web-devicons`. Wincent's hand-rolled-statusline path (zero plugins, `vim.opt.statusline` + `lua/wincent/statusline.lua`) was considered and declined since the lualine swap is ~10 lines for the aesthetic-integration win, vs. real config writing for the wincent path. Main draw was theme integration — `theme = 'auto'` inherits everforest colors automatically, where airline's themes felt stuck in 2015. | — | 🔵 | 2 |
| File explorer | **In progress 2026-06-06** — leaning toward **nvim-tree.lua** as the traditional-sidebar pick (NERDTree successor, same paradigm, smaller dep footprint than neo-tree). **neo-tree.nvim rejected 2026-06-06** after side-by-side eval — heavier deps (`plenary.nvim` + `nui.nvim`), larger config surface, and the right-aligned-dot aesthetic that initially attracted us wasn't worth the cost given fzf already covers multi-source views (`:Files` / `:GFiles?` / `:Buffers`). If nvim-tree doesn't work out, fallback is reverting to NERDTree (`Plug 'scrooloose/nerdtree'` is parked commented in `vimrc.org`). | **oil.nvim** still queued for separate paradigm evaluation after harpoon — see Next-up section below. | 🔵 | 2 |
...
| Polyglot syntax | **⏳ Queued for evaluation (2026-06-06).** Treesitter is wired and active across the languages we touch daily, but `vim-polyglot` is still in the plugin list — likely double-sourcing syntax for languages where both have coverage, and only carrying its weight for the (shrinking) set treesitter doesn't parse yet. Wincent's pattern is the reference: **dropped polyglot entirely; uses treesitter + targeted single-language plugins** (`rust.vim`, `vim-ansible-yaml`, `vim-git`, `vim-zsh`) for the cases treesitter doesn't cover well. Action when picked up: list the languages polyglot's `g:polyglot_disabled` would need to whitelist, cross-reference with our treesitter parsers + any specific quirks (filetype detection, indent rules, ftdetect entries), then drop polyglot's `Plug` line and add only the targeted survivors. | 🟡 | 2 (re-prioritized 2026-06-06) |
...
| Diagnostic UI | **⏸ Passed 2026-05-25** after pitching. Reframed: native nvim 0.11+ diagnostics already cover the workflow — `[d`/`]d` navigation (default 0.10+), `vim.diagnostic.open_float()` on `CursorHold`, `vim.diagnostic.setqflist()` for workspace-wide → quickfix, `vim.lsp.buf.references()`/`definition()` already land in quickfix, severity filtering via `vim.diagnostic.config()`. trouble.nvim would add a *panel* (grouped-by-file collapse, severity tinting, preview-on-cursor in a unified buffer) — UX polish, not new capability. User's minimalism preference + wincent precedent (he also skips it, lives in quickfix-land) make the weight unjustified. Re-evaluate if a concrete pain point emerges (e.g. wanting all workspace diagnostics visible at a glance without `:copen`). | **trouble.nvim** is the candidate if re-picked; folke ecosystem, polished, popular. | 🔵 | deferred — pending concrete pain |
...
| Org-mode editor support | User edits `vimrc.org` (the dotfiles' own literate source) and other `.org` files regularly. `vim-polyglot` provides basic syntax highlight but no editing primitives. **Primary felt need: better outline navigation** — heading cycling, fold/expand, jump-to-heading. Full survey of org capabilities (TODOs, agenda, capture, links) also desirable while we're in here. | **nvim-orgmode/orgmode** is the headliner — actively-maintained Lua port of Emacs org-mode, covers outline cycling (`<TAB>`), TODO state, agenda views, capture, link follow. Optional pairing: **nvim-orgmode/telescope-orgmode.nvim** for fuzzy heading picker (would only matter if we're already on Telescope; if we stay on fzf, can ship a similar wrapper later). **nvim-neorg/neorg** uses its own `.norg` format — not a fit, would require migrating the dotfiles' literate source away from .org. | 🔵 | 2 |
...
- **Effectively done.** Steps 1–3 landed: treesitter `main` API (commit `5c9fd08a`), nvim-compe → nvim-cmp + LuaSnip (`6967fcf6`), LSP wiring with 12 servers via mason-lspconfig + the nvim 0.11 `vim.lsp.config()` / `vim.lsp.enable()` API (`a9bfd845`). `:checkhealth mason` clean on a fresh tmux server (stale-PATH gotcha hit once and resolved with `tmux kill-server`).
...
Remaining Phase 2 candidates, by tier:

- 🔵 **File explorer — traditional sidebar (in progress).** NERDTree → **nvim-tree.lua** (neo-tree.nvim **rejected** 2026-06-06; see inventory row). Committed checkpoint at `e7174bbb` / `9eaca037` (2026-06-06) — actively running and usable, but pending refinement on git-marker rendering before category close-out. **Open tweaks queued for next session:**
  - **Try moving git-dirty signals to end of line** instead of nvim-tree's default left-of-filename placement. This is the one architectural gap vs neo-tree; nvim-tree doesn't have a first-class right-aligned slot, so would need a custom renderer component or `statuscolumn` hack. Worth investigating whether feasible without forking.
  - **Or keep git markers as-is** (left-side glyphs) if the move-to-right path turns into a swamp. Acceptable fallback.
  - **Update the git-marker symbol set** regardless of position — defaults are letter-prefix style (`M`, `A`, `??`, etc.) which read as noisy. Configurable via `renderer.icons.glyphs.git = {...}`.
  - **Stretch goal: minimalist colored-bar markers matching the gitsigns gutter.** The gitsigns sign-column uses solid vertical bars (`│` / `▎`-style) tinted yellow for modified, green for added, etc. Best case is making nvim-tree's git column visually consistent with that — a single tinted character per file rather than a letter prefix. Likely path: set each `renderer.icons.glyphs.git.<status>` to the same bar character and rely on nvim-tree's highlight groups (`NvimTreeGitDirty`, `NvimTreeGitNew`, `NvimTreeGitDeleted`, etc.) for the colors. Investigate which highlights nvim-tree exposes and whether they inherit from gitsigns' palette automatically.
- 🔵 **Org-mode editor support** — currently `vim-polyglot` syntax-only. **Primary need: better outline navigation in `.org` files** (heading cycle, fold/expand, jump-to-heading). Headliner: `nvim-orgmode/orgmode`. See inventory row above.
- 🟡 **Quick-nav marks (harpoon) — ⏸ passed (2026-06-06).** Walked the pitch and declined. Reasoning: existing `<Leader><Leader>` (alternate file, depth-1) + `<Leader>b` (fzf buffer picker) cover most of harpoon's territory, and the middle ground where slot-addressing wins big over fuzzy-on-buffer-list is narrow for the user's typical buffer counts (≤5-10). Additionally — and the dealbreaker — harpoon's UI presents pinned files as a tab-like pinned strip, and the user already gets that signal from tmux window naming. Adding it inside nvim duplicates the signal and adds visual noise. Re-visit only if buffer counts routinely exceed 20 AND fzf's filter-to-the-4-files-I-want becomes a recurring friction.
- 🟡 **Oil.nvim — ⏸ passed (2026-06-06).** Walked the pitch and declined. Reasoning: oil's main leverage is *batch* filesystem operations from inside nvim (mass rename, cross-directory moves, bulk reorganize), and in 2026 the workflow for non-trivial batch ops has shifted to AI agents that do dry-runs and explain what they did — agents now consume oil's main use case. What remains (single-rename, edit-path-as-text, quick "what's in this dir" peek) is adequately covered by `nvim-tree`'s `r` / `d` keys, `:e dir/`, and fzf's `:Files`. The plugin itself is well-built and trustworthy (same author as `conform.nvim` which we adopted happily), so revisit is cheap if a concrete pain point ever surfaces — but for the current workflow, the cost (mental surface, one more plugin to track, one more `-` binding to defend) outweighs the marginal value.
- 🟡 **Evaluate dropping `vim-polyglot`** — queued 2026-06-06. Treesitter is wired and active; polyglot still installed, likely double-sourcing syntax. Wincent's pattern: drop polyglot, keep treesitter + targeted single-language plugins (`rust.vim`, `vim-ansible-yaml`, `vim-git`, `vim-zsh`) for non-treesitter cases. See inventory row above for the migration steps.
- 🟡 Fuzzy finder (fzf vs Telescope vs fzf-lua), Movement (flash/leap), Folding (nvim-ufo), Indent guides (indent-blankline), Better git diff UI (diffview.nvim).
...
This session may resume on a different machine. Everything that matters is in this file + the commits referenced above; no per-machine state was created. The Mac entrypoint is `home-mac.nix`; Linux is `home.nix`; both `imports = [ ./common.nix ]`. Re-tangle workflow + privacy guard + tmux-resurrect failure modes are in `.claude/CLAUDE.md`.

The existing TODOs in `.claude/todos/todos.md` (nvim-cmp/treesitter migration; LSP wiring) are now subsumed by Phase 1 here. Those entries should be replaced with one-line pointers to this file (deferred to next session pickup, to keep this file's introduction self-contained).

### Resumption prompt — Claude should open the next session with this question, verbatim

> File explorer Phase 2 work is mid-stream — NERDTree replaced with nvim-tree.lua (committed `e7174bbb` + `9eaca037`), running and usable, but with queued tweaks before category close-out. Per the Next-up section, the standing tweaks are: (1) explore moving git-dirty signals to end-of-line in nvim-tree (architecturally tricky — no first-class right-aligned slot, may need custom renderer/`statuscolumn`); (2) update git-marker symbol set regardless of position (defaults are letter-style, `renderer.icons.glyphs.git = {...}` is the knob); (3) stretch goal — make nvim-tree's git column visually consistent with the gitsigns gutter (single colored bar character per file, tinted via `NvimTreeGitDirty` / `NvimTreeGitNew` / etc. highlight groups). After file-explorer wraps, the queue is harpoon → oil.nvim paradigm eval → other 🟡/⚪ items. Resume by picking one of the three tweaks; if (1) turns into a swamp, fall back to keeping markers left-aligned and focusing on (2) + (3). Which tweak first?
