...
- 🔵 **File explorer — traditional sidebar (in progress).** NERDTree → **nvim-tree.lua** (neo-tree.nvim **rejected** 2026-06-06; see inventory row). Refining nvim-tree config — fallback if it doesn't suit: revert to NERDTree (commented `Plug` is parked in `vimrc.org`). Oil.nvim deferred to separate evaluation — see below.
...
> Phase 1 is effectively done; four Phase 2 categories landed across the last two sessions (linter/formatter → conform.nvim, status line → lualine with diff+diagnostics, diagnostic UI → passed in favor of native, theme refresh → Kanagawa Dragon end-to-end with Rose Pine Moon queued as alternative). Ready to continue Phase 2? Remaining 🔵 candidates: **file explorer** (NERDTree → oil.nvim ± nvim-tree.lua — biggest cognitive shift) and **org-mode editor support** (`nvim-orgmode/orgmode` — primary need is outline nav in .org files since user edits `vimrc.org` regularly; added 2026-05-31). Other options are the 🟡 row (fuzzy finder, movement, harpoon, folding, indent guides, diffview) or the ⚪ taste-driven set (cmdline overhaul, outline, marginal trio). Which category should we walk?
