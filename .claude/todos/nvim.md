...
| Polyglot syntax | **✅ Done 2026-06-06.** Dropped `vim-polyglot` entirely with no targeted-replacement plugins. Treesitter (24 parsers covering all daily-driver languages: bash/c/clojure/css/diff/fish/go/html/javascript/json/lua/markdown/markdown_inline/nix/python/query/rust/toml/tsx/typescript/vim/vimdoc/yaml + `org` via nvim-orgmode) + LSP (12 servers) cover everything we use. Nvim's built-in ftdetect handles all our file extensions correctly; the `.org` filetype force-autocmd we added as a polyglot-workaround is also removed now that polyglot's general detection isn't shadowing nvim's. Wincent's "targeted survivors" pattern (`rust.vim`, `vim-ansible-yaml`, `vim-git`, `vim-zsh`) was considered and skipped — we'd add them lazily when concrete breakage surfaces, not pre-emptively. Risk surface noted in case anything regresses: `commentstring` for niche filetypes (used by vim-commentary's `gc`), indent rules for languages without an attached LSP, ftdetect for outliers like terraform `.tf`. None observed in initial testing. | — | 🟡 | 2 |
