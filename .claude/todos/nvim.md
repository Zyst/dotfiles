...
- **Phase 1 step 4 is next**: add `minuet-ai.nvim` with the Anthropic provider for AI tab completion. Open question still on the table from the Decisions section: try minuet's Anthropic backend first vs. start with Supermaven/Copilot to establish a latency baseline and switch later. Secrets-management approach for the Anthropic API key also needs to be picked — likely a `~/.config/op/...` 1Password CLI fetch, a `~/.config/...env` git-ignored file, or `home.sessionVariables` reading from a non-tracked path.
- Step 5 (jj module) remains queued after.
...
- **Resumption prompt — Claude should open the next session with this question, verbatim:**
  > Ready to continue Phase 1? Step 4 (minuet-ai.nvim + Anthropic provider) is queued next; step 5 (jj module) follows. Two open questions to decide before installing: (a) start with minuet's Anthropic backend or with a fast baseline like Supermaven/Copilot and switch later; (b) how to plumb the Anthropic API key (1Password CLI, git-ignored env file, or `home.sessionVariables` reading from a non-tracked path).
