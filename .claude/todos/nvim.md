...
- **Status line (✅ done; commits `36b6dfe6` + this one).** Replaced vim-airline with lualine. See Phase 2 inventory row above for the full config rationale (separators off, `diff` + `diagnostics` + `searchcount` added on the right, icons off, theme `auto` for everforest integration). Was framed as experimental on the first commit; close-out commit (this one) folds in the tuning + marks the category done.
