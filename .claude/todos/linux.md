...
- **Priority shift (2026-05-24 evening):** Deprioritized further. The user expects to SSH into this Linux box often (Mac is the primary daily driver), so polishing the local GUI matters less than it would for an in-person daily driver. Sway is still wanted — just no rush. Current higher priority: bring nvim to full power (LSP wiring + nvim-cmp migration — see `todos.md`). Pick the Sway thread back up once nvim is in a good place.
